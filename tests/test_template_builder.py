...
def test_build_price_cross_ma_produces_valid_pine():
    spec = BuilderSpec(
        title="Test EMA Cross", script_type="indicator",
        pattern=SignalPattern.PRICE_CROSS_MA, primary_indicator="ema",
    )
...
def test_build_ma_crossover_uses_both_indicators():
    spec = BuilderSpec(
        title="Test Cross", script_type="indicator",
        pattern=SignalPattern.MA_CROSSOVER, primary_indicator="ema",
...
def test_build_oscillator_threshold_uses_custom_levels():
    spec = BuilderSpec(
        title="Test RSI", script_type="indicator",
        pattern=SignalPattern.OSCILLATOR_THRESHOLD, primary_indicator="rsi",
...
def test_build_band_breakout_produces_upper_lower():
    spec = BuilderSpec(
        title="Test BB", script_type="indicator",
        pattern=SignalPattern.BAND_BREAKOUT, primary_indicator="bb",
    )
    source = build_from_spec(spec)
...
def test_build_strategy_with_fixed_percent_risk():
    spec = BuilderSpec(
        title="Test Strategy", script_type="strategy",
        pattern=SignalPattern.PRICE_CROSS_MA, primary_indicator="sma",
        risk=RiskPreset.FIXED_PERCENT, stop_loss_pct=3.0, take_profit_pct=6.0,
    )
    source = build_from_spec(spec)
...
def test_build_strategy_with_atr_risk():
    spec = BuilderSpec(
        title="Test Strategy", script_type="strategy",
        pattern=SignalPattern.PRICE_CROSS_MA, primary_indicator="sma",
        risk=RiskPreset.ATR_BASED, atr_stop_mult=1.5, atr_tp_mult=2.5,
    )
    source = build_from_spec(spec)
...
def test_build_indicator_type_has_alerts_not_strategy_entries():
    spec = BuilderSpec(
        title="Test Ind", script_type="indicator",
        pattern=SignalPattern.PRICE_CROSS_MA, primary_indicator="sma",
    )
    source = build_from_spec(spec)
...
def test_every_indicator_lints_clean_in_its_simplest_valid_pattern(indicator_id):
    ...
    spec = BuilderSpec(
        title="Lint Check", script_type="indicator", pattern=pattern,
        primary_indicator=indicator_id, overlay=spec_ind.is_overlay_friendly,
    )
    source = build_from_spec(spec)
...
def test_every_indicator_appears_in_at_least_one_pattern():
    ...
    for indicator_id, _spec_ind in INDICATORS.items():
        found = any(
            any(c.id == indicator_id for c in indicators_for_pattern(p))
